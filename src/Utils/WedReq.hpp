    ...
    arc::TaskHandle<void> WebReq(std::string const& url, matjson::Value const& params, matjson::Value const& bodyJSON, Callback&& cb) {
        ...
        return async::spawn(
            ...
            [cb = std::forward<Callback>(cb), url](web::WebResponse res) {
                if (!res.ok()) {
                    if (res.code() == -1) {
                        ...
                        if (wrappedJSON.isErr()) {
                            std::string errorBody = res.string().unwrapOrDefault();
                            log::error("HTTP Error {} from '{}'. Body is not JSON: {}", res.code(), url, errorBody);
                            cb(matjson::Value::object(), {APIErrorType::JSONError, APIMessage::None});
                            return;
                        }

                        auto json = wrappedJSON.unwrap();
                        if (json.isObject() && json.contains("message") && json["message"].isString()) {
                ...
                if (json.isObject() && json.contains("data")) {
                    cb(std::move(json["data"]), {});
                } else {
                    cb(std::move(json), {});
                }
            }
